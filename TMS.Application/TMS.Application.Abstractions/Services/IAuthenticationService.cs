...
namespace TMS.Application.Abstractions.Services
{
    ...
    public interface IAuthenticationService
    {
        ...
        /// <summary>
        /// Authenticates or registers a user via Telegram and returns an authentication result.
        /// </summary>
        /// <param name="dto">The DTO containing Telegram authentication data.</param>
        /// <param name="cancellationToken">A token that can be used to cancel the operation.</param>
        /// <returns>
        /// A task that represents the asynchronous authentication operation. 
        /// The task result contains the authentication result (token, user info, success status, error messages, etc.).
        /// </returns>
        Task<AuthenticationResultDto> AuthenticateTelegramAsync(TelegramAuthenticationDto dto, CancellationToken cancellationToken = default);
    }
}